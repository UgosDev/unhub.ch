    ...
    return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
            try {
                ...
                // In caso di errore, rigetta per usare l'immagine originale
                reject(error);
            }
        };
...
function AuthenticatedApp() {
  ...
  const handleMoveDocumentsToModule = useCallback(async (group: DocumentGroup, targetModule: 'archivio' | 'polizze' | 'disdette', options?: { isPrivate?: boolean }) => {
    ...
    const handleDeleteAddressBookEntry = useCallback(async (id: string) => {
        if (confirm("Sei sicuro di voler eliminare questo contatto dalla rubrica? L'azione è irreversibile.")) {
            try {
                await db.deleteAddressBookEntry(id);
            } catch (e) {
                const errorMessage = e instanceof Error ? e.message : String(e);
                setError(`Errore durante l'eliminazione del contatto: ${errorMessage}`);
            }
        }
    }, []);
        ...
        switch(currentPage) {
            ...
            case 'dashboard':
                return <Dashboard
                    ...
                    addressBook={addressBook}
                    onDeleteAddressEntry={handleDeleteAddressBookEntry}
                />;
            case 'guide':
