...
// FIX: Add missing function suggestProcessingMode
    ...
    try {
        ...
        const analysisSchema = {
            ...
            properties: {
                insights: {
                    type: Type.ARRAY,
                    description: "Una lista di 2-3 suggerimenti o osservazioni utili basate sull'analisi complessiva delle polizze (es. coperture duplicate, possibili risparmi, polizze mancanti).",
                    items: { type: Type.STRING }
                },
                upcomingRenewals: {
                    type: Type.ARRAY,
                    description: "Una lista di polizze che scadono nei prossimi 90 giorni, ordinate per data.",
                    items: {
                        type: Type.OBJECT,
                        properties: {
        ...
        const prompt = `Sei un consulente assicurativo esperto. Analizza il seguente portafoglio di polizze assicurative. Fornisci suggerimenti e identifica le scadenze imminenti. La data odierna è ${new Date().toISOString().split('T')[0]}.

Portafoglio polizze (JSON):
...
`;
        const response = await ai.models.generateContent({
            ...
            contents: prompt,
            config: {
                systemInstruction: "Rispondi SOLO con l'oggetto JSON richiesto dallo schema. Sii conciso e pratico.",
                responseMimeType: "application/json",
        ...
        throw new Error("L'analisi AI non è riuscita. Riprova più tardi.");
    }
};

// FIX: Add missing function startUgoExperienceStream
export const startUgoExperienceStream = async (base64Data: string, onChunk: (chunk: any) => void): Promise<void> => {
    const ugoExperienceSchema = {
        type: Type.OBJECT,
        properties: {
            userFeedback: {
                type: Type.STRING,
                description: "Breve feedback per l'utente (es. 'Avvicinati', 'Tieni fermo', 'Buona luce')."
            },
            documentCorners: {
                type: Type.ARRAY,
                description: "Array di 4 punti [{x, y}] con le coordinate normalizzate (0.0-1.0) degli angoli del documento. Ordine: tl, tr, br, bl. Se non vedi un documento, restituisci un array vuoto.",
                items: {
                    type: Type.OBJECT,
    ...
    try {
        const responseStream = await ai.models.generateContentStream({
            ...
            config: {
                systemInstruction: "Sei un assistente per la scansione di documenti. Analizza l'immagine in tempo reale e fornisci feedback per aiutare l'utente a ottenere uno scatto perfetto. Rispondi solo con frammenti di JSON.",
                responseMimeType: "application/json",
        ...
        for await (const chunk of responseStream) {
            try {
                const parsedChunk = JSON.parse(chunk.text);
                onChunk(parsedChunk);
            } catch (e) {
        ...
        console.error("Ugo Experience stream failed:", error);
    }
};
