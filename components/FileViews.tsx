...
interface FileViewsProps {
    ...
    onConfirmProcessing: (mode: ProcessingMode) => void;
...
export const PendingFilesView: React.FC<Pick<FileViewsProps, 'pendingFiles' | 'suggestedMode' | 'isSuggestingMode' | 'processingMode' | 'onProcessingModeChange' | 'shouldExtractImages' | 'onShouldExtractImagesChange' | 'onConfirmProcessing' | 'onCancelProcessing'>> = (props) => {
    return (
        <div className="w-full flex flex-col p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-xl">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 px-1 mb-2">Documenti Pronti</h3>
            <div className="bg-slate-100 dark:bg-slate-900/50 p-3 rounded-lg max-h-32 overflow-y-auto space-y-1">
                {props.pendingFiles.map(file => (
                    <p key={file.name} className="text-sm text-slate-600 dark:text-slate-300 truncate">{file.name}</p>
                ))}
            </div>

            <div className="mt-4">
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 px-1 mb-2">2. Scegli la Modalità</h3>
                {props.isSuggestingMode ? (
                    ...
                    <ProcessingModeSelector 
                        ...
                        disabled={false}
                        suggestedMode={props.suggestedMode}
                    />
                )}
                 {props.suggestedMode && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 px-2 flex items-center gap-1.5">
                        <InformationCircleIcon className="w-4 h-4 flex-shrink-0" />
                        <span>Abbiamo suggerito una modalità, ma puoi cambiarla se necessario.</span>
                    </p>
                )}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row gap-2">
                ...
                <button onClick={() => props.onConfirmProcessing(props.processingMode)} className="px-4 py-2 font-bold bg-purple-600 text-white rounded-lg w-full sm:flex-grow">
                    Elabora {props.pendingFiles.length} file
                </button>
            </div>
        </div>
    );
