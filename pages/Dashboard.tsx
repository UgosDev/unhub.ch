...
import type { ProcessedPageResult, ScanHistoryEntry, ProcessingMode, AddressBookEntry } from '../services/geminiService';
import { DocumentTextIcon, ShieldCheckIcon, ChevronRightIcon, CoinIcon, BoltIcon, DownloadIcon, CreditCardIcon, CheckCircleIcon, XCircleIcon, SparklesIcon, ArrowUturnLeftIcon, UsersIcon, TrashIcon } from '../components/icons';
...
interface DashboardProps {
    ...
    onDeleteAddressEntry: (id: string) => void;
}
...
const Dashboard: React.FC<DashboardProps> = ({ user, onNavigate, history, isLoadingHistory, addressBook, onDeleteAddressEntry }) => {
    ...
    return (
        <div className="flex flex-col gap-8 max-w-5xl mx-auto">
            ...
            <div className="bg-white dark:bg-slate-800/50 p-6 rounded-2xl shadow-lg">
                <h2 className="font-bold text-xl text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-3">
                    <UsersIcon className="w-6 h-6 text-purple-600 dark:text-purple-400" />
                    Rubrica Indirizzi
                </h2>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                    Contatti estratti automaticamente dalle tue scansioni. Utili per pre-compilare le disdette.
                </p>
                <div className="max-h-72 overflow-auto space-y-3 pr-2">
                    {addressBook.length > 0 ? (
                        addressBook.map(entry => (
                            <div key={entry.id} className="bg-slate-50 dark:bg-slate-700/50 p-3 rounded-md flex items-center justify-between gap-4 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700">
                                <div>
                                    <p className="font-semibold text-slate-700 dark:text-slate-200">{entry.name}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 whitespace-pre-wrap">{entry.address}</p>
                                </div>
                                <button
                                    onClick={() => onDeleteAddressEntry(entry.id)}
                        ...
                        <div className="text-center py-8">
                            <p className="text-slate-500 dark:text-slate-400">La tua rubrica è vuota.</p>
                            <p className="text-sm text-slate-400 dark:text-slate-500">Inizia a scansionare per popolarla automaticamente.</p>
                        </div>
                    )}
                </div>
            </div>
