...
import { PolizzeChLogoIcon, PolizzeChWordmarkIcon, DocumentTextIcon, SparklesIcon, UsersIcon, ShieldCheckIcon, DocumentPlusIcon, XMarkIcon, PaperAirplaneIcon, MagnifyingGlassIcon } from '../components/icons';
...
const Polizze: React.FC<PolizzeProps> = ({ polizzeDocs, archivedDocs, consultants }) => {
    ...
    const uniqueCompanies = useMemo(() => {
        const companies = new Set(polizzeDocs.map(doc => doc.analysis.soggetto || 'Sconosciuta'));
    ...
    const filteredDocs = useMemo(() => {
        let docs = polizzeDocs;

        if (selectedCompanies.length > 0) {
            docs = docs.filter(doc => selectedCompanies.includes(doc.analysis.soggetto || ''));
        }

        if (filterQuery.trim()) {
            const lowerCaseQuery = filterQuery.toLowerCase();
            docs = docs.filter(doc => 
                doc.analysis.soggetto?.toLowerCase().includes(lowerCaseQuery)
            );
        }
        
        return docs;
    }, [polizzeDocs, filterQuery, selectedCompanies]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    <div className="lg:col-span-2 space-y-4">
                         <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div className="relative w-full sm:w-64">
                                ...
                                <input
                                    ...
                                    placeholder="Filtra per compagnia..."
                                    className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-800 rounded-full border border-slate-300 dark:border-slate-600 focus:ring-2 focus:ring-cyan-500"
                                />
                            </div>
                         </div>
                        
                        {uniqueCompanies.length > 0 && (
                            <div className="flex flex-wrap items-center gap-2 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg">
                                <span className="text-sm font-semibold text-slate-600 dark:text-slate-300 mr-2">Compagnie:</span>
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1 text-xs font-bold rounded-full transition-colors ${
                                            selectedCompanies.includes(company)
                                                ...
                                                : 'bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-200 hover:bg-slate-200'
                                        }`}
                                    >
                                        {company}
                                    </button>
                                ...
                                {selectedCompanies.length > 0 && (
                                    <button onClick={() => setSelectedCompanies([])} className="text-xs font-semibold text-red-500 hover:underline ml-auto">
                                        Pulisci
                                    </button>
                                )}
                            </div>
                        ...
                        {polizzeDocs.length === 0 ? (
                            ...
                            <div className="text-center p-10 bg-white dark:bg-slate-800/50 rounded-2xl">
                                <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200">Nessun risultato</h3>
                                <p className="text-slate-500 dark:text-slate-400">Nessuna polizza corrisponde ai filtri selezionati.</p>
                            </div>
                        )}
                    </div>
